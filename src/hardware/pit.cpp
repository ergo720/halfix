...
itick_t pit_next(itick_t now)
{
    if (pit.chan[0].timer_running) {
        ...
        uint32_t raise_irq = 0;

        // checking (count > pit.chan[0].pit_last_count) will miss the case where the counter wrapped around but the new count is less than or equal to pit_last_count
        itick_t next_time, pit_period = pit_counter_to_itick(pit.chan[0].count);
        if (now - pit.chan[0].last_irq_time >= pit_period) {
            // Count has gone from 0 --> 0xFFFF
            pit.chan[0].last_irq_time = now;
            next_time = pit_period; // interrupt time period
            raise_irq = 1;
        }
        else {
            next_time = pit.chan[0].last_irq_time + pit_period - now; // time remaining until next interrupt
        }
